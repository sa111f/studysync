...
public class AiExamParserService {
    ...
    private String systemPrompt(LocalDate today, ZoneId zone) {
        ...
        return "You extract exam information from student text.\n\n"
             ...
             + "Today's date is " + isoToday + " (a " + dayName + ").\n"
             + "The current day of the week is " + dayName + ".\n"
             + "Current time is " + time + ". The user's timezone is " + zone.getId() + ".\n\n"
             + "When resolving relative dates, follow these rules EXACTLY:\n"
             + "- \"today\" → today's date\n"
             + "- \"tomorrow\" → today + 1 day\n"
             + "- \"this <weekday>\" → the next occurrence of that weekday in the current week (if today is past it, use next week)\n"
             + "- \"next <weekday>\" → the <weekday> in the NEXT calendar week (Monday-Sunday week), NOT just \"the next occurrence\"\n"
             + "- \"next week\" → any day in the week that starts the Monday after this current week\n"
             + "- \"next week <weekday>\" → that specific weekday in next week (Monday-Sunday week starting after the upcoming Sunday)\n"
             + "- \"in N days\" → today + N days\n"
             + "- \"in N weeks\" → today + (N * 7) days\n"
             + "- \"end of the week\" → upcoming Friday\n"
             + "- \"end of next week\" → Friday of next week\n\n"
             + "Before returning, VERIFY by computing the day-of-week for your chosen dateTime and confirming it matches what the user asked for. If the user said \"Monday\" the date MUST be a Monday.\n\n"
             + "Example: If today is Tuesday April 28, 2026, then \"next week Monday\" = 2026-05-04 (Monday). NOT May 8 (Friday).\n\n"
